...
Controller.prototype.handleRequest = function (direction, floor) {
    ...
    var j, minIndex = -1, minDistance = Infinity;
    for (j = 0; j < main.elevators.length; j++) {
        var distanceIndex = main.elevators[j].isEligible (direction, floor);
        if (distanceIndex >= 0 && distanceIndex < minDistance) {
            minIndex = j; minDistance = distanceIndex;
        }
    }
    if (minIndex != -1) {
...
Elevator.prototype.statusDisplay = function () {
    this.html = '';
    if (this.direction == 0) {
        this.html = 'Floor: ' + this.idleFloor + '<br/>' + 'Door Opened' + '<br>'; 
        var destination = prompt("Enter Destination Floor");   
    } else {
        this.html =  'Moving in ' + ((this.direction == 1) ? 'up' : 'down') + ' direction' + '<br>' + 'Door Closed' + '<br>';    
